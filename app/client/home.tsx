...
export default function ClientMapHome() {
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      // 🚨 DISPARAR NOTIFICACIÓN PUSH REAL AL DISPOSITIVO DEL PROVEEDOR
      if (selectedProvider.expoPushToken) {
        await sendPushNotification(
          selectedProvider.expoPushToken,
          "¡NUEVO SERVICIO SOLICITADO! 🚨",
          `El cliente ${user?.email?.split('@')[0] || 'Nuevo'} requiere tus servicios. Abre la app para aceptar.`
        );
      }
    ...
    <View style={styles.container}>
      ...
      {activeRequest?.status === 'ACCEPTED' && (
        <View style={[styles.bottomSheet, { backgroundColor: colors.card, paddingBottom: 40 }]}>
          ...
          <TouchableOpacity style={{ alignSelf: 'center', marginTop: 20 }} onPress={cancelRequest}>
