    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={[styles.iconButton, { backgroundColor: colors.card }]} onPress={async () => { try { await signOutWithNotifications(); router.replace('/auth/login'); } catch { Alert.alert('No se pudo cerrar sesión', 'Revisa tu conexión e intenta nuevamente.'); } }}><Ionicons name="log-out-outline" size={23} color={colors.danger} /></TouchableOpacity>
        </View>
        ...
        {tab === 'QUEUE' ? requests.map((request) => (
          <View key={request.id} style={[styles.card, { backgroundColor: colors.card, borderColor: request.priority === 'HIGH' ? '#E74C3C' : colors.border }]}>
            ...
            {request.evidence_photo ? <><Text style={{ color: colors.subtext, marginTop: 12 }}>Evidencia del trabajo</Text><Image source={{ uri: request.evidence_photo }} resizeMode="contain" style={styles.issuePhoto} /></> : null}
          </View>
